...
object ArgsFormatChecker {
  ...
  val aipmRpcServerIpPattern = "((2[0-4]\\d|25[0-5]|[01]?\\d\\d?)\\.){3}(2[0-4]\\d|25[0-5]|[01]?\\d\\d?):[0-9]{1,5}".r
  ...
  def isValid(arg: Any, formatName: String): Boolean = {
    formatName match {
      ...
      case "aipmRpcServerIp" => aipmRpcServerIpPattern.pattern.matcher(arg.toString).matches()
      case _ => false
    }
  }
